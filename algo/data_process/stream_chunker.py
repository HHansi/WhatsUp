...
def filter_documents_by_time(input_filepath: str, from_time_str: str, to_time_str: str,
    ...
    if output_filepath:
        ...
        # df = pd.DataFrame(columns=['Timestamp', 'Text-Original'])
        ...
        if row[1] != "_na_":
            ...
            if (time >= from_time) and (time <= to_time):
                ...
                if output_filepath:
                    ...
                    i += 1
    # df.to_excel(output_filepath, sheet_name='Sheet1', index=False)
    if output_filepath:
        output_file.close()
    return n
...
def filter_documents_by_time_bulk(from_time_str: str, to_time_str: str, time_duration: int, input_file_path: str,
    ...
    while from_time_temp < to_time:
        ...
        # output_file_path = os.path.join(output_folder_path, short_from_time_str + '.xlsx')
