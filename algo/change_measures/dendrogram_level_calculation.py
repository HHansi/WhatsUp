...
def get_normalized_dendrogram_level_similarity(label1, label2, label_codes, max_level_count):
    ...
    :return: float, int, int
        ...
        Number of shared dendrogram levels between label1 and label2
        Maximum number of levels in the dendrogram
    """
    # if word is not in dendrogram raise KeyError
    if label1 not in label_codes or label2 not in label_codes:
        raise KeyError('Given label is not found in the dendrogram.')
    ...
    if len(code1) == len(code2) and len(common_code) == len(code1):
        return 1, len(code1), max_level_count
    return len(common_code) / max_level_count, len(common_code), max_level_count


def get_local_dendrogram_level_similarity(label1, label2, label_codes):
    """
    Method to calculate Local Dendrogram-Level (LDL) similarity
    ...
    :return: float, int, int
        DL similarity between label1 and label2
        Number of shared dendrogram levels between label1 and label2
        Maximum number of levels between the targeted labels in the dendrogram
    """
    # if word is not in dendrogram raise KeyError
    if label1 not in label_codes or label2 not in label_codes:
        raise KeyError('Given label is not found in the dendrogram.')
    ...
    # find max length code
    max_length_code = code1
    if len(code2) > len(max_length_code):
        max_length_code = code2

    # similarity between word itself is 1
    if len(code1) == len(code2) and len(common_code) == len(code1):
        return 1, len(code1), max_length_code
    return len(common_code) / len(max_length_code), len(common_code), len(max_length_code)
